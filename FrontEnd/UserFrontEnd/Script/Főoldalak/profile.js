...
export async function loadProfileData() {
    ...
    if (!userId) {
        fillProfileFields(email, authUser.fullName ?? authUser.FullName ?? "", authUser.billingAddress ?? authUser.BillingAddress ?? "");
        return;
    }
    setCurrentUserId(userId);
    const profile = await fetchUserProfile(userId);
    fillProfileFields(profile?.email ?? profile?.Email ?? email, profile?.fullName ?? profile?.FullName ?? "", profile?.billingAddress ?? profile?.BillingAddress ?? "");
}
