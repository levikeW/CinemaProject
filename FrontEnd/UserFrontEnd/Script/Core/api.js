...
export async function fetchJegyekList() {
...
export async function fetchSeatsForRoom(roomId, screeningId) {
...
export async function fetchImages(movieId) {
...
export async function fetchScreeningTickets(screeningId) {
    try {
        const response = await fetch(`${API_BASE}/api/cinema/getticketsbyscreening?screeningId=${encodeURIComponent(String(screeningId))}`, { credentials: "include" });
        if (!response.ok) {
            return [];
        }
        const payload = await response.json().catch(() => null);
        const rows = normalizeCollectionPayload(payload);
        const result = [];
...
// Regisztráció
export async function registerUser(email, fullName, password, billingAddress) {
...
export async function fetchUserProfile(userId) {
    try {
        const response = await fetch(`${API_BASE}/api/user/viewprofile?userId=${encodeURIComponent(String(userId))}`, { credentials: "include" });
        if (!response.ok) {
            return null;
        }
        const payload = await response.json().catch(() => null);
...
export async function removeServerCart(cartId) {
    try {
...
function getReservationStorageKey(userId) {
    return `${reservationStorageKeyPrefix}:${userId}`;
}
function getHiddenReservationStorageKey(userId) {
    return `${hiddenReservationStorageKeyPrefix}:${userId}`;
}
function normalizeReservationSeatLabels(seats) {
    if (!Array.isArray(seats)) {
        return [];
    }
    const result = [];
...
export function fetchStoredReservations(userId) {
    ...
    try {
        ...
        for (let i = 0; i < parsedReservations.length; i++) {
            const reservation = parsedReservations[i];
...
export function fetchHiddenReservationIds(userId) {
...
export function removeStoredReservation(userId, paymentReservationId) {
    const storedReservations = fetchStoredReservations(userId);
    const nextReservations = [];
    for (let i = 0; i < storedReservations.length; i++) {
...
export function hideReservationLocally(userId, paymentReservationId) {
...
export function restoreHiddenReservation(userId, paymentReservationId) {
    const hiddenReservationIds = fetchHiddenReservationIds(userId);
    const nextHiddenReservationIds = [];
    for (let i = 0; i < hiddenReservationIds.length; i++) {
        if (hiddenReservationIds[i] !== paymentReservationId) {
...
export async function fetchServerCart(userId) {
    try {
        const response = await fetch(`${API_BASE}/api/cart/getcart?userId=${encodeURIComponent(String(userId))}`, { credentials: "include" });
        if (!response.ok) {
            return [];
        }
        const payload = await response.json().catch(() => null);
...
export async function createServerReservation(cartId) {
    try {
...
export async function deleteReservationOnServerByPaymentId(paymentReservationId) {
    try {
...
export async function fetchUpcomingReservations(userId) {
    try {
        const response = await fetch(`${API_BASE}/api/payment_reservation/viewupcomingreservation?userId=${encodeURIComponent(String(userId))}`, { credentials: "include" });
        if (!response.ok) {
            return [];
        }
        const payload = await response.json().catch(() => null);
        const items = normalizeCollectionPayload(payload);
        const result = [];
...
export async function fetchPastReservations(userId) {
    try {
        const response = await fetch(`${API_BASE}/api/payment_reservation/viewpastreservation?userId=${encodeURIComponent(String(userId))}`, { credentials: "include" });
        if (!response.ok) {
            return [];
        }
        const payload = await response.json().catch(() => null);
...
export async function fetchReservationConfirmation(reservationId) {
    try {
        const response = await fetch(`${API_BASE}/api/payment_reservation/getconfirmation?reservationId=${encodeURIComponent(String(reservationId))}`, { credentials: "include" });
        if (!response.ok) {
            return null;
        }
        return await response.json().catch(() => null);
